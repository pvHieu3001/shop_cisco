...
import { useGetProductsQuery, useUpdateProductMutation } from '../../../../../services/ProductsEndpoints'
...
export default function ListProduct() {
  ...
  const columns: TableProps<IProduct>['columns'] = [
    ...
    },
    {
      ...
      render: (thumbnail) => (
        <div
          className=' rounded-md w-[40px] h-[40px] overflow-hidden '
          style={{ boxShadow: 'rgba(1, 1, 1, 0.06) 1rem 1.25rem 1.6875rem 1rem' }}
        >
        ...
        <Flex wrap='wrap' gap='small'>
          ...
          {!!data.is_active && (
            <Popconfirm
              ...
              onConfirm={() => confirm(String(data.slug))}
              okText='Yes'
              cancelText='No'
            >
              <Button danger loading={isHiddenProduct && data.id == id}>
                Disable
              </Button>
            </Popconfirm>
          )}
