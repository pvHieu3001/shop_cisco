              ...
              <tbody>
                {variant[0].attribute.map((parent, index) =>
                  index == 0 || index < variant[0].attribute.length - 1 ? (
                    <tr
                      className='bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-[#f6f6f6] dark:hover:bg-gray-600'
                      key={index}
                    >
                      <th
                        scope='row'
                            ...
                            >
                              {parent.url ? (
                                <div style={{ height: '100%', maxWidth: '100%' }} className='relative group'>
                                  ...
                                  ></div>
                                  <DeleteOutlined
                                    onClick={() => handelRemoveImage(parent.id)}
                                    className=' duration-1000 opacity-0 group-hover:opacity-100 absolute left-[50%] top-[50%]'
                                    style={{
                      ...
                      {variant.length < 2 ? (
                        <>
                          ...
                          <td className='px-6 py-4'>
                            <Form.Item name={[parent.id, 'sku']} className='m-0' help=''>
                        ...
                        <>
                                    ...
                                    rules={[
                                      ...
                                      ({ getFieldValue }) => ({
                                        validator(_, value) {
                                          if (
                                            !value ||
                                            getFieldValue(['variant', parent.id + '-' + item.id, 'price']) > value
                                          ) {
                                            return Promise.resolve()
                                          }
                                          return Promise.reject(new Error('Giá khuyến mãi phải nhỏ hơn giá bán'))
                          ...
                          <td className='px-6 py-4'>
                            {variant[1].attribute.map((item, key) =>
                              key == 0 || key < variant[1].attribute.length - 1 ? (
                                <div className='block py-3' key={key}>
                                  <Form.Item name={[parent.id + '-' + item.id, 'sku']} className='m-0' help=''>
