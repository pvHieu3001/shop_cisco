...
const DATA_colors = [{ name: 'Nâu' }, { name: 'Đen' }, { name: 'Vàng' }]
...
const DATA_sortOrderRadios = [
  ...
  { name: 'Giá thấp - cao', id: 'asc' },
  { name: 'Giá cao - thấp', id: 'desc' }
]
...
const TabFilters = (props: Props) => {
  ...
  const [rangePrices, setRangePrices] = useState([1, 100])
  ...
  const handleChangeMaterial = (checked: boolean, name: string) => {
    checked ? setmaterialState([...materialState, name]) : setmaterialState(materialState.filter((i) => i !== name))
  }
        ...
        {({ open, close }) => (
          <>
            ...
            >
              ...
              <span className='ml-2'>
                {sortOrderStates ? DATA_sortOrderRadios.filter((i) => i.id === sortOrderStates)[0].name : 'Sắp xếp'}
              </span>
              {!sortOrderStates.length ? (
                <ChevronDownIcon className='w-4 h-4 ml-3' />
              ) : (
          ...
          <>
            ...
            >
              ...
              {!materialState.length ? (
                ...
                <span onClick={() => setmaterialState([])}>{renderXClear()}</span>
              )}
            </Popover.Button>
            <Transition
              as={Fragment}
                    ...
                    {DATA_material.map((item) => (
                      <div key={item.name} className=''>
                        <Checkbox
                          name={item.name}
                          label={item.name}
                          defaultChecked={materialState.includes(item.name)}
                          onChange={(checked) => handleChangeMaterial(checked, item.name)}
                        />
                      </div>
                    ))}
                  </div>
        ...
        {({ open, close }) => (
          <>
            ...
            >
              ...
              <span className='ml-2 min-w-[90px]'>{`${rangePrices[0]}tr - ${rangePrices[1]}tr`}</span>
                              ...
                              <div>
                                ...
                                <div className='mt-1 relative rounded-md'>
                                  <div className='absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none'>
                                    <span className='text-neutral-500 sm:text-sm'>tr</span>
                                  </div>
                                  <input
                                    type='text'
                                    name='minPrice'
