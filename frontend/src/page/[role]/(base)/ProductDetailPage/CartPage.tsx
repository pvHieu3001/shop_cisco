...
const CartPage = () => {
  ...
  const renderProduct = (item: ICart) => {
    const { id, price, name, slug, thumbnail, quantity, price_sale } = item
    const product = {
      id: id,
      price,
      price_sale,
    ...
    return (
      <div key={item.id} className='relative flex py-8 sm:py-10 xl:py-12 first:pt-0 last:pb-0'>
    ...
    <div className='nc-CartPage'>
      ...
      <main className='container py-16 lg:pb-28 lg:pt-20 '>
        <div className='mb-12 sm:mb-16'>
          <h2 className='block text-2xl sm:text-3xl lg:text-4xl font-semibold '>Giỏ hàng</h2>
        ...
        {!isLoading && !dataCart?.length && (
          <Result
            icon={<CartEmptyAnimationIcon width='200px' height='200px' />}
            title='Giỏ hàng trống'
            extra={
              <Link to='/'>
                <Button type='primary' key='console' className=' bg-black !rounded-[20px]'>
                  Quay về trang chủ
                </Button>{' '}
              </Link>
            }
        ...
        <div className='flex flex-col lg:flex-row'>
          <div className='w-full lg:w-[60%] xl:w-[55%] divide-y divide-slate-200 dark:divide-slate-700 '>
            {isLoading ? <h1> Đang tải</h1> : dataCart?.map(renderProduct)}
          </div>

          {!isLoading && Boolean(dataCart?.length) && (
            <>
                ...
                <div className='sticky top-28'>
                  ...
                  <div className='mt-7 text-sm text-slate-500 dark:text-slate-400 divide-y divide-slate-200/70 dark:divide-slate-700/80'>
                    ...
                    <div className='flex justify-between font-semibold text-slate-900 dark:text-slate-200 text-base pt-4'>
                      <span>Tổng</span>
                      <span>{dataCart && VND(getTotalPriceCart(dataCart))}</span>
                    </div>
                  </div>
